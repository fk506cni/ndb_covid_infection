...
-- O2
dpc_o2 as (
  SELECT pick_si_seq2('tnds_t_rcp_dpc_si', {ym}, {o2_code}) as seq2_no
  UNION
  SELECT pick_cd_seq2('tnds_t_rcp_dpc_cd', {ym}, {o2_code}) as seq2_no
),

-- Int
dpc_int as (
  SELECT pick_si_seq2('tnds_t_rcp_dpc_si', {ym}, {int_code}) as seq2_no
  UNION
  SELECT pick_cd_seq2('tnds_t_rcp_dpc_cd', {ym}, {int_code}) as seq2_no),

-- ecmo
dpc_ecmo as (
  SELECT pick_si_seq2('tnds_t_rcp_dpc_si', {ym}, {ecmo_code}) as seq2_no
  UNION
  SELECT pick_cd_seq2('tnds_t_rcp_dpc_cd', {ym}, {ecmo_code}) as seq2_no),
...
dpc_adm_gen as (
  ...
  UNION
  SELECT pick_cd_seq2('tnds_t_rcp_dpc_cd', {ym}, {adm_gen}) as seq2_no),

-- men admin
dpc_adm_men as (
  SELECT pick_si_seq2('tnds_t_rcp_dpc_si', {ym}, {adm_men}) as seq2_no
  UNION
  SELECT pick_cd_seq2('tnds_t_rcp_dpc_cd', {ym}, {adm_men}) as seq2_no),
...
dpc_adm_unit_ad as (
  ...
  UNION
  SELECT pick_cd_seq2('tnds_t_rcp_dpc_cd', {ym}, {adm_unt_ad}) as seq2_no),
...
dpc_adm_unit_man as (
  ...
  UNION
  SELECT pick_cd_seq2('tnds_t_rcp_dpc_cd', {ym}, {adm_unt_man}) as seq2_no),
...
dpc_re_lim as (
  SELECT 
    ...
    --o2
    CASE WHEN seq2_no in (
        SELECT seq2_no  FROM dpc_o2
      ) THEN 1
      ELSE 0 END as had_o2,
  
    --int
    CASE WHEN seq2_no in (
        SELECT seq2_no  FROM dpc_int
      ) THEN 1
      ELSE 0 END as had_int,
      
    --ecmo
    CASE WHEN seq2_no in (
        SELECT seq2_no  FROM dpc_ecmo
      ) THEN 1
      ELSE 0 END as had_ecmo,
    
    
    --adm_gen
    CASE WHEN seq2_no in (
        SELECT seq2_no  FROM dpc_adm_gen
      ) THEN 1
      ELSE 0 END as had_adm_gen,
    
    --adm_men
    CASE WHEN seq2_no in (
        SELECT seq2_no FROM dpc_adm_men
      ) THEN 1
      ELSE 0 END as had_adm_men,
    
    --adm_unit
    CASE WHEN seq2_no in (
        SELECT seq2_no FROM dpc_adm_unit_ad
      ) THEN 1
      ELSE 0 END as had_adm_unit_ad,
      
    --adm_unit
    CASE WHEN seq2_no in (
        SELECT seq2_no FROM dpc_adm_unit_man
      ) THEN 1
...
-- O2
med_si_o2 as (
SELECT pick_si_seq2('tnds_t_rcp_med_si', {ym}, {o2_code}) as seq2_no),

-- Int
med_si_int as (
SELECT pick_si_seq2('tnds_t_rcp_med_si', {ym}, {int_code}) as seq2_no),

-- ecmo
med_si_ecmo as (
SELECT pick_si_seq2('tnds_t_rcp_med_si', {ym}, {ecmo_code}) as seq2_no),
...
med_re_lim as (
  SELECT 
    ...
    0 as had_covid_sb,
        ...
        --o2
    CASE WHEN seq2_no in (
        SELECT seq2_no  FROM med_si_o2
      ) THEN 1
      ELSE 0 END as had_o2,
  
    --int
    CASE WHEN seq2_no in (
        SELECT seq2_no  FROM med_si_int
      ) THEN 1
      ELSE 0 END as had_int,
      
    --ecmo
    CASE WHEN seq2_no in (
        SELECT seq2_no  FROM med_si_ecmo
      ) THEN 1
      ELSE 0 END as had_ecmo,
...
re_smr as (
  SELECT
      ...
      CASE 
        WHEN sum(had_o2) > 0 THEN 1
        ELSE 0 END as had_o2_whole,
      
      CASE 
        WHEN sum(had_int) > 0 THEN 1
        ELSE 0 END as had_int_whole,
      
      CASE 
        WHEN sum(had_ecmo) > 0 THEN 1
        ELSE 0 END as had_ecmo_whole,

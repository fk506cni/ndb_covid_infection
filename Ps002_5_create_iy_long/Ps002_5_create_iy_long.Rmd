---
title: "Ps002_5_create_dpciy_long"
author: "fk506cni"
date: "2023/6/22"
...
df_filter <- function(df, tb){
  ...
  error_file <- c("./error_",tb,"_", get_ts(), ".rds") %>% 
    str_c(collapse = "")
  if(str_detect(tb, "_iy")){
      if(! "medicine_cd" %in% colnames(df)){
      saveRDS(df, error_file)
      }else{
      df <- df %>% 
        dplyr::filter(medicine_cd %in% drugcode) %>%
        mutate(
            use_amnt_str = if_else(is.na(use_amnt), "NA", format(use_amnt,nsmall=5)) %>% 
              str_remove_all(" ")) %>% 
        dplyr::select(-one_of(c("cmt1_cd","char1_dat", "cmt2_cd", "char2_dat", "cmt3_cd","char3_dat",
...
```{r}
format(c(1.9, 4312.123), nsmall=5, nlarge=8)
```
...
```{r}
f_create <- "./queries/create_iy_long.sql"
...
rcon <- f2con(f_r)
  ...
  # 引用元はtnds_t_rcp_dpc_iy。このｔｂは飲み込み先
  tb <- "tnds_t_rcp_dpc_iy_long"
  schm <- "osr_ndb_user"
  f <- "./queries/select_iy_long.sql"
...
clusterExport(cl, varlist = c("df", "df_grid","drugcode"), envir = environment())

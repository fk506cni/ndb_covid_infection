...
f_ <- "./queries/covid_hoko_id_smr_any_wfunc.sql"
# %>% 
#   .[14:14]
...
get_tabl <- function(i){
  ...
  print(ym)
  arglist$ym <- ym
  log_out(i = i, l = arglist, tag = "hoko_tabl")
  df_ym_i <-  qfa2df(q_f = f_, arglist = arglist, con = con)
  df_ym_i <- df_ym_i %>%
      mutate(yms = ym,
             file = f_)
  
  log_out(i = i, l = arglist, tag = "hoko_tabl", timing = "end")
  gc(reset = T)
  gc(reset = T)
  return(df_ym_i)
}
...
```{r}
f_out <- c("./covid_hoko_res_",get_ts(), ".rds") %>% 
  str_c(., collapse = "")
f_out
res %>%

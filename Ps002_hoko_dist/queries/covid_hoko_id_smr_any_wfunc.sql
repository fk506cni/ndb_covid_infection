...
ho_lim as (
  SELECT *
  FROM tnds_t_rcp_whole_ho
  WHERE prac_ym = {ym}
),

rehoko_with as (
  SELECT
    re_with_cov.id1n as id1n
    ,age_grade
    ,had_covid_bool
    ,coalesce(ho_lim.sum_total_score,0) as sum_total_score
  FROM re_with_cov 
  LEFT JOIN ho_lim ON re_with_cov.id1n = ho_lim.id1n
...
re_with_cov_smr as (
  SELECT
    ...
    ,sum(sum_total_score) as sum_of_sum_total_score
  FROM
    rehoko_with
  GROUP BY had_covid_bool
  ,age_grade
)
--,coalesce(phakh.total_score,0)
--select * from re_with_cov;
--select * from re_lim;
--select * from re_with_cov ;
--select * from rehoko_with;
